...
# Pacific cod
taxa.2.use <- "Gadidae"
pacific_cod$p_pcrit <- calc_many_p_po2(temp = pacific_cod$temperature_C,
                                       po2 = pacific_cod$po2,
                                       taxa.name = taxa.2.use,
                                       w = W)
pacific_cod$cpue_weight <- with(pacific_cod, catch_weight / effort)

# Pacific Hake
pacific_hake$p_pcrit <- calc_many_p_po2(temp = pacific_hake$temperature_C,
                                       po2 = pacific_hake$po2,
                                       taxa.name = taxa.2.use,
                                       w = W)

pacific_hake$cpue_weight <- with(pacific_hake, catch_weight / effort)

# Pacific Halibut
taxa.2.use <- "Pleuronectidae"
pacific_halibut$p_pcrit <- calc_many_p_po2(temp = pacific_halibut$temperature_C,
                                        po2 = pacific_halibut$po2,
                                        taxa.name = taxa.2.use,
                                        w = W)
...
# Dover Sole
taxa.2.use <- "Pleuronectidae"
dover_sole$p_pcrit <- calc_many_p_po2(temp = dover_sole$temperature_C,
                                           po2 = dover_sole$po2,
                                           taxa.name = taxa.2.use,
                                           w = W)
