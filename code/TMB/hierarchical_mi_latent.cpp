#include <TMB.hpp>
  ...
  Type objective_function<Type>::operator() ()
  {
    ...
    PARAMETER( beta_method ); // effect of paper method on measured pcrit
    PARAMETER_VECTOR(beta_source); // latent effect of paper on measued pcrit
    PARAMETER( logsigma );
    PARAMETER( logsigmap );
    ...
    // NLL from random effects (beta_source)
    jnll_comp( 0 ) = -sum( dnorm( beta_source, 0, sigmap, true) );  
   ...
   Type covmult;
    ...
    // Probability of the data, accounting for traits 
    for( int id=0; id<n_d; id++){
     mu( id ) =  Eo( taxa_id( id ) ) * invtemp( id ) + n_pow( taxa_id( id ) ) * logW( id  ) - log(V( taxa_id( id ) )) - beta_source( sourceno( id ) ) - beta_method * method_mat( id );
    }
