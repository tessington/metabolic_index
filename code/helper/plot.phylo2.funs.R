boxtext <- function(x, y, labels = NA, col.text = NULL, col.bg = NA, 
                    border.bg = NA, adj = NULL, pos = NULL, srt = 0, offset = 0.5, 
          ...
          ...) 
{
  ...
  if (plot) {
      ...
      if (phyloORclado) {
        ...
        if (lab4ut == "horizontal") {
          ...
          text(xx.tips + x.adj * cex, yy.tips + y.adj * 
               ...
               srt = srt, cex = cex, col = tip.color, bg = "white")
        }
        else {
          if (align.tip.label) {
    ...
    if (show.node.label) 
      boxtext(xx[ROOT:length(xx)] + label.offset, yy[ROOT:length(yy)], 
           x$node.label, adj = adj, font = font, srt = srt, 
           cex = cex, col.bg = "white")
  }
