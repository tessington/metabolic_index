...
```
Generally, the center of the simulated species-level distribution of traits within Classes are close to the overall mean values (Ao = 3.41, Eo = 0.350), and 95% of species in these classes will fall between 1.95 and 4.88 (logAo) and 0.036 and 0.66 (Eo).  Ao is often more easily interpreted as its inverse, which depicts the $pO_{2,crit}$ of a species at the reference temperature (15 C) and at a reference body size.  This, critical $pO_{2,crit}$ will range from 0.00754 atm to 0.1422 atm, and this range is similar among all classes.

There were slight differences among Orders (only Orders with at least 3 separate species are shown), mostly manifest as slightly different distributions of $\log(A_o)$,but projected ranges of species traits were still largely similar
...
Slightly larger differences in species-level traits were projected among some families, although many of these groups included only two species (three families had three species represented). Zoocaridae (eelpouts) and Gadidai (cods) had relatively low logAo (or lower tolerance for low $pO2$), while species in the families Penaidae and Triptyrygiidae had a higher logAo (higher tolerance).  There was more variability in $E_o$ among families, with estimated temperature dependence being low among Salmonidae and exceptionally high among Penaeidae.

```{r plotfam, echo = F, fig.cap = "Same as above but for Family, restricted to Families represented by at least two species"}
...
## Point 3: most of the trait variation is at lower taxonomic levels

```{r getlambda, echo = F}
log_lambda_table <- log_lambda
rownames(log_lambda_table) <- c("Order", "Family", "Species")
knitr::kable(log_lambda_table, caption = "Estimated (SE) of log-lambdas by taxonomic group")

```

The estimates of $\lambda_{l(j)}$ indicate increasing trait variance with lower taxonomic level, at odds with the interpretation of these terms as the proportion of evolutionary trait variation attribuble to that taxonomic level  This may be either due to:

* localized, species level adaptation to different environments to take advantage of available niches (e.g. deeper and more oxygen depleted habitats)

* differences in experimental protocols among studies that drive many of the differences in estimated $pO_{2,crit}$


## Point 4: Hierarchical model improves estimation through shrinkage

Because not all data have mass data (and when present, mass and temperature are often not independent across experiments), I fit individual linear models to each species, using the maximum likelihood estimate of n = -0.078, and assigning an offset of $n \log(W)$ to account for relative body mass.  This was set to 0 when body mass was absent.


Fitting individual linear model to species-level data typically leads to wider variability of traits than hierarchical models. In other words, the hierarchical model dismisses exceptionally unusual parameter estimates to a species data and "shrinks" those estimates to the Parent group level means.  In essence, this stabilizes parameter estimates, and may thereby provide a better basis for generating probability distributions of trait values. 


```{r compare, echo = F, fig.cap = "Comparison of parameter estimates derived without and with hierarchical models"}
...
p1 <- ggplot(SpeciesEst, aes(x = Aoind, y = Ao_atm)) +
  geom_point() + 
  geom_abline(a = 0, b = 1) +
...
p2 <- ggplot(SpeciesEst, aes(x = Eoind, y = Eo)) +
  geom_point() + 
  geom_abline(a = 0, b = 1) + 
